...
const BuyActions: FC<Props> = ({ locale, offerId, quantity, section, data, onSubmit }) => {
	...
	return (
		<div className='buttons-buy flex flex-col items-end gap-2'>
			{ cartItems.find(item => +item.id === offerId) ?
				<Button color='success' size='lg' radius='sm' onPress={ onOpen } className='uppercase w-full font-bold lg:w-72'>
					{ t('in cart') }
				</Button> :
				<Button onPress={ handleClickBuy } color='primary' radius='sm' size='lg' className='uppercase w-full font-bold lg:w-72'>
					{ t('buy') }
				</Button>
			}
			<QuickOrder
				...
				<ModalContent>
					{ (onClose) => (
						<>
							...
							<ModalFooter>
								<Button variant='bordered' size='lg' className='uppercase font-bold hidden lg:block text-black dark:text-gray-50' radius='sm' onPress={onClose}>
									{ t('continue shopping') }
								</Button>
								<Button color='primary' size='lg' className='uppercase font-bold' radius='sm' onPress={ handleClick }>
									{ t('place an order') }
								</Button>
							</ModalFooter>
						</>
