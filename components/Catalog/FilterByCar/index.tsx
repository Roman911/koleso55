		...
		<div className='flex justify-end items-center lg:items-start mb-3'>
			...
			<div className="relative inline-block text-left z-20">
				<button type="button" onClick={() => setOpenSort(prev => !prev)}
								className="w-56 xl:w-64 h-11 p-3 flex items-center justify-between bg-white text-xs uppercase font-bold border border-[#C4C8CC] rounded-sm text-black"
								id="menu-button">
					<div>{t(sort)}</div>
				...
				<div
					className={twMerge('absolute right-0 z-10 w-56 xl:w-64 origin-top-right border border-[#C4C8CC] bg-white shadow-lg p-3 xl:p-5 rounded-sm text-black', !openSort && 'hidden' )}
					tabIndex={-1}>
